...
EMOTION_MODEL_MAX = int(os.getenv("EMOTION_MODEL_MAX", "500"))  # Increased from 200
...
# ------------------------------
# IMPROVED Emotion model with emoji detection
# ------------------------------
try:
...
def analyze_emoji_sentiment(text):
    ...
    neutral_count = sum(text.count(e) for e in neutral_emojis)
    
    total_emojis = pos_count + neg_count + neutral_count
    
    if total_emojis == 0:
        return None  # No emoji sentiment detected
    
    if pos_count > neg_count and pos_count > neutral_count:
        return {"emotion": "happy", "sentiment": "positive", "confidence": 0.7}
    elif neg_count > pos_count:
        return {"emotion": "sad", "sentiment": "negative", "confidence": 0.7}
...
def analyze_text_heuristic(text):
    """Enhanced heuristic analysis with more patterns and lower threshold"""
    text = safe_text(text)
    if not text:
        return {"emotion": "unclear", "sentiment": "neutral", "confidence": 0.0}
    
    text_lower = text.lower()
    
    # First check emoji sentiment
    emoji_result = analyze_emoji_sentiment(text)
    if emoji_result:
        return emoji_result
    
    # Positive patterns - expanded
    positive_patterns = [
        r'\b(love|amazing|awesome|wonderful|fantastic|great|excellent|perfect|beautiful|brilliant|best|happy|joy|excited|thrilled|blessed|grateful|thankful|festive|merry|jolly|celebration|celebrate|fun|enjoy|magic|magical|special)\b',
        r'\b(can\'t wait|so excited|looking forward|absolutely love|really love|adore|obsessed)\b',
        r'[!]{2,}',  # Multiple exclamation marks indicate excitement
        r'\b(yay|woohoo|yess|omg|wow)\b'
    ]
    
    # Negative patterns - expanded
    negative_patterns = [
        r'\b(hate|terrible|awful|horrible|worst|bad|sad|disappointed|frustrat|annoying|annoyed|angry|mad|stress|stressful|exhausted|tired|broke|expensive|chaos|panic|worried|anxiety|overwhelm)\b',
        r'\b(don\'t like|can\'t stand|fed up|had enough|too much|so done)\b',
        r'\b(ugh|urgh|ffs|wtf)\b'
    ]
    
    # Neutral/nostalgic patterns
    nostalgic_patterns = [
        r'\b(remember|childhood|memories|used to|back in|old days|nostalgic|throwback|classic|traditional)\b'
    ]
    
    # Shopping/gift patterns (context-dependent)
    shopping_patterns = [
        r'\b(shopping|gift|present|buy|bought|spending|queue|queues|crowded|busy|sale|sales|boxing day)\b'
    ]
    
    # Cooking/baking patterns
    cooking_patterns = [
        r'\b(baking|cooking|recipe|pavlova|cookies|cake|dinner|feast|food prep)\b'
    ]
    
    positive_score = sum(len(re.findall(p, text_lower, re.I)) for p in positive_patterns)
    negative_score = sum(len(re.findall(p, text_lower, re.I)) for p in negative_patterns)
    nostalgic_score = sum(len(re.findall(p, text_lower, re.I)) for p in nostalgic_patterns)
    shopping_score = sum(len(re.findall(p, text_lower, re.I)) for p in shopping_patterns)
    cooking_score = sum(len(re.findall(p, text_lower, re.I)) for p in cooking_patterns)
    
    # Christmas context always slightly positive unless negative words present
    christmas_boost = 0
    if 'christmas' in text_lower or 'xmas' in text_lower:
        christmas_boost = 1
    
    # Determine sentiment based on scores
    if nostalgic_score > 0:
        return {"emotion": "nostalgic", "sentiment": "neutral", "confidence": 0.75}
    
    if positive_score + christmas_boost > negative_score:
        emotion = "happy"
        if cooking_score > 0 or shopping_score > 0:
            emotion = "excited"
        return {"emotion": emotion, "sentiment": "positive", "confidence": 0.7}
    
    if negative_score > positive_score + christmas_boost:
        emotion = "stressed"
        if shopping_score > 0:
            emotion = "frustrated"
        return {"emotion": emotion, "sentiment": "negative", "confidence": 0.7}
    
    # If we have shopping or cooking context but no clear sentiment, assume positive
    if shopping_score > 0 or cooking_score > 0:
        return {"emotion": "busy", "sentiment": "neutral", "confidence": 0.6}
    
    # Default to neutral for Christmas-related content
    if christmas_boost > 0:
        return {"emotion": "festive", "sentiment": "neutral", "confidence": 0.5}
    
    return {"emotion": "unclear", "sentiment": "neutral", "confidence": 0.4}


def analyze_text_with_model(text, model_pipeline=None, min_confidence=0.45):
    """Use ML model with lower confidence threshold and emoji backup"""
    text = safe_text(text)
    if not text:
        return {"emotion": "unclear", "sentiment": "neutral", "confidence": 0.0}
    
    # Always check emoji sentiment first
    emoji_result = analyze_emoji_sentiment(text)
        ...
        try:
            ...
            if isinstance(preds, list) and preds:
                ...
                if conf >= min_confidence:
                    sentiment = "positive" if emotion in ("happy", "generous", "excited") else ("negative" if emotion in ("sad", "angry", "worried") else "neutral")
                    return {"emotion": emotion, "sentiment": sentiment, "confidence": conf}
                elif emoji_result:
                    # Model uncertain but we have emoji sentiment
                    return emoji_result
        except Exception:
            pass
    
    # Fall back to heuristic or emoji
    if emoji_result:
        return emoji_result
...
def process_records(records, use_model=True, max_items=500):
    ...
    for i, raw in enumerate((records or [])[:max_items]):
        ...
        if use_model and model_pipeline and i < EMOTION_MODEL_MAX:
            analysis = analyze_text_with_model(txt, model_pipeline=model_pipeline, min_confidence=0.45)
        else:
